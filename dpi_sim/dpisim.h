...
extern "C" uint64_t get_csr_from_rtl(int);
extern "C" void     set_csr_in_rtl(int, uint64_t);
extern "C" void     end_rtl_simulation(void);

// this class represents one processor in a RISC-V machine.
class dpisim_t: public processor_t
{
public:
	dpisim_t(
	    sim_t*    _sim,
...
//
  //virtual void set_pcr(int which, reg_t val)
  virtual void set_fromhost(reg_t val);
  virtual void set_interrupt(int which, bool on);
  //virtual reg_t get_pcr(int which);
  ...
  bool check_state(state_t* micro_state, state_t* isa_state, db_t* actual);

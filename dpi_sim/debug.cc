...
bool debug_buffer_t::run_ahead(){
  bool htif_return = true;

  ifprintf(logging_on,stderr, "Functional simulator running ahead\n");
...
void debug_buffer_t::push_state_actual(state_t* a_state_ptr,bool checkpoint_state){
  ...
  if(checkpoint_state){
    ...
    db[tail].a_state->pc                =  a_state_ptr->pc;
...
void debug_buffer_t::dump(dpisim_t* proc, db_t* actual, FILE* file)
{

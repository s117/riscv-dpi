...
void processor_t::take_interrupt()
{
  ...
  ifprintf(logging_on,stderr,"Taking interrupt status: 0x%x irqs: 0x%x\n", state.sr, irqs);
...
static reg_t execute_insn(processor_t* p, reg_t pc, insn_fetch_t fetch)
{
  ...
  p->update_histogram(pc);
    ...
    if(p->get_checker()){
	    ...
	    p->get_pipe()->push_state_actual(p->get_state(),true);
    }
  #endif
  return npc;
}
  ...
  catch(trap_t& t)
  {
    ...
    pc = take_trap(t, pc);
      ...
      if(get_checker()){
	      ...
	      get_pipe()->push_state_actual(&state,true);
      }
    #endif
  }
  catch(serialize_t& s) {
...
{
  ifprintf(logging_on,stderr,"Set PCR 0x%x -> 0x%lx\n",which,val);
  switch (which)
  {
    case CSR_FFLAGS:
...
{
  //fprintf(stderr,"In set interrupt\n");
